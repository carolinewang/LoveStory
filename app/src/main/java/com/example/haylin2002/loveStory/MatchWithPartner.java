...
//                        partnerUser.put("partner",currentUser.getUsername());
                                                currentUser.saveInBackground();
                                                partnerUser.saveInBackground(new SaveCallback() {
                                                    @Override
                                                    public void done(ParseException e) {
                                                        Toast.makeText(MatchWithPartner.this,
                                                                "Successfully matched with user: " +
                                                                        partnerUser.getUsername() +
                                                                "You both need to match with each other to confirm", Toast.LENGTH_SHORT).show();
                                                    }
                                                });
                                                Intent intent = new Intent(MatchWithPartner.this, LoveStory.class);
